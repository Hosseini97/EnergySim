...
def create_sample_data(n_days: int = 7):
    ...
    heeten_load_w = np.resize(heeten_df["load"].to_numpy(dtype=float), n_steps) * WATTS_PER_KW
    heeten_irradiance_w_m2 = np.resize(heeten_df["pv"].to_numpy(dtype=float), n_steps) * WATTS_PER_KW
    ...
    df = pd.DataFrame({
        ...
        "solar_irradiance_w_m2": heeten_irradiance_w_m2,
